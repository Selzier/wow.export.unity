	...
	async export(dir, quality) {		
		...
		for (let x = 0, midX = 0; x < 16; x++) { 	// Chunk X
			for (let y = 0; y < 16; y++) { 			// Chunk Y
				...
				for (let row = 0, idx = 0; row < 17; row++) { // Subchunk X
					const isShort = !!(row % 2);
			...
			for (let x = 0; x < 16; x++) { 	// Subchunk X
				...
				for (let row = 0; row < ySize; row++) {
					for (let col = 0; col < xSize; col ++) {
						...
						heightArraySubchunk[yPos][xPos] = vertexHeightList[index]; // Save the subchunk height data to a small 2d array
					}
				}
				...
				for (var a=0; a < heightArraySubchunk.length; a++){
					...
					for (var b=0; b < xRow.length; b++){
						heightArray2d[y*16 + b][x*16 + a] = heightArraySubchunk[a][b]; // Add the small 2d array into the larger sized full chunk array
					}
				}
		...
		for (var a=0; a < heightArray2d.length; a++){ heightArray2d[a].reverse(); }
		var jsonPath = path.join(dir, 'adt_' + this.tileID + '_HeightData.json');
		returnArray[4] = jsonPath;
		try { fs.writeFileSync(jsonPath, JSON.stringify(heightArray2d)); } catch (err) { log.write(err); }
		//************************************************************************//
		...
		if (quality !== 0) {
			...
			if (quality === -2) {
				...
				var imageCount = Math.ceil(materialIDs.length/4); 			// How many splatmaps will be generated for this chunk
				var pixelData = new Array(imageCount); 						// pixelData is an ARRAY of splatmap pixel data (multiple splatmaps)
				for (var p = 0; p < pixelData.length; p++) {				// For each splatmap, initialize a 1024x1024 2d array 
					pixelData[p] = new Uint8ClampedArray(1024 * 1024 * 4); 	// 1024*1024*4 because each pixel has an RGBA byte
				}
					...
					for (let y = 0; y < 16; y++) {
						...
						if (textureLayers.length > 0) {
							...
							for (let j = y * bytesPerColumn; j < (y * bytesPerColumn) + bytesPerColumn; j += bytesPerRow) {
								// Now we need to loop the x axis. 64 pixels long, 4 bytes each = 256
								for (let i = x * bytesPerSubRow; i < (x * bytesPerSubRow) + bytesPerSubRow; i += bytesPerPixel) {
									...
									// Looping the subchunk's texture layers
									for (var k = 0; k < textureLayers.length; k++) {
				...
				for (var t = 0; t < pixelData.length; t++) {
					var myPath = path.join(tgaPath, 'splatmap_' + this.mapDir + "_" + prefix + '_' + t.toString() + '.tga');
				...
				if (splitTextures || config.overwriteFiles || !await generics.fileExists(tileOutPath)) {
					...
					for (let x = 0; x < 16; x++) {
						for (let y = 0; y < 16; y++) {
							...
							gl.uniform4f(uHeightScale, ...heightScales); // Value does not affect splat order
							gl.uniform4f(uHeightOffset, ...heightOffsets); // Value does not affect splat order
					...
					if (!splitTextures) { await saveCanvas(path.join(dir, 'tex_' + this.tileID + '.png')); } // Save the completed tile.
					gl.bindBuffer(gl.ARRAY_BUFFER, null); // Clear buffer.
					for (const mat of materials) { gl.deleteTexture(mat.texture); } // Delete loaded textures.
				}
			}
		}
